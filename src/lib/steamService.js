...
export async function fetchSteamUserLibrary(steamIdInput) {
  ...
  // Extract 17-digit SteamID64 from any URL or plain input
  ...
  const isValidXml = (text) =>
    typeof text === 'string' && (text.includes('<profile>') || text.includes('<error>'));

  let xmlContent = null;

  // Strategy 1: Supabase Edge Function with Steam Web API (full library, no CORS — most reliable)
  try {
    const param = isNumeric ? `steamid=${cleanedId}` : `vanity=${cleanedId}`;
    const fnUrl = `${SUPABASE_URL}/functions/v1/steam-profile?${param}`;
    const res = await fetch(fnUrl, {
    ...
    if (res.ok) {
      ...
      if (data.ownedAppIds?.length) {
        return {
          success: true,
          steamId: data.steamId || cleanedId,
          personaName: data.personaName || cleanedId,
          avatar: data.avatar || null,
          ownedAppIds: data.ownedAppIds,
          totalOwned: data.totalOwned || data.ownedAppIds.length,
        };
      }
    }
  } catch (_) {}

  // Strategy 2: Vite local dev proxy
  if (!xmlContent) {
    try {
      const localUrl = isNumeric
        ? `/api/steamcommunity/profiles/${cleanedId}?xml=1`
        : `/api/steamcommunity/id/${cleanedId}?xml=1`;
      const res = await fetch(localUrl, { signal: AbortSignal.timeout(4000) });
      if (res.ok) {
        const text = await res.text();
        if (isValidXml(text)) xmlContent = text;
      }
    } catch (_) {}
  }
  ...
  if (!xmlContent) {
    try {
      const targetUrl = isNumeric
        ? `https://steamcommunity.com/profiles/${cleanedId}?xml=1`
        : `https://steamcommunity.com/id/${cleanedId}?xml=1`;
      const res = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(targetUrl)}`, {
        signal: AbortSignal.timeout(8000),
      });
      if (res.ok) {
        const json = await res.json();
        if (isValidXml(json?.contents)) xmlContent = json.contents;
      }
    } catch (_) {}
  }
  ...
  if (xmlContent.includes("<error>")) {
    ...
    if (rawError.toLowerCase().includes("private") || rawError.toLowerCase().includes("hidden")) {
      return { success: false, error: "This Steam profile is Private. Please set 'Game Details' to 'Public' in your Steam Privacy Settings." };
    }
  ...
  if (xmlContent.includes("<privacyState>private") || xmlContent.includes("<privacyState>friendsonly")) {
    return { success: false, error: "This Steam profile's games are set to Private or Friends Only. Please set 'Game Details' to 'Public' in your Steam Privacy Settings." };
  }
  ...
  const appMatches = [...xmlContent.matchAll(/\/app\/(\d+)|<statsName>(?:<!\[CDATA\[)?(\d+)(?:\]\]>)?<\/statsName>/g)];
  const ownedAppIds = [...new Set(appMatches.map(m => m[1] || m[2]))].filter(Boolean);

  if (!ownedAppIds.length) {
    return { success: false, error: "No games found on this profile, or your Steam 'Game Details' privacy is not set to Public." };
  }
